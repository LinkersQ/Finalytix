...
create table if not exists public.tmp_1week_union_history_candles_all_scales
(
    scale             varchar,
    figi              varchar,
    candle_start_dt_utc   timestamp,
...
comment on column public.tmp_1week_union_history_candles_all_scales.type_of_candle is 'Свеча восходящая (U) / Свеча нисходящая (D) / Свеча нейтральная (N)';
alter table public.tmp_1week_union_history_candles_all_scales
    owner to postgres;
...
INSERT INTO public.tmp_1week_union_history_candles_all_scales
(
    scale,
    figi,
    candle_start_dt_utc,
...
(
    SELECT
        '1_week_scale' as scale,
        figi,
        date_trunc('week',public.union_history_candles.candle_start_dt_utc) as candle_start_dt_utc,
        ...
        'script_for_1_week_intervals_intervals' as source_filename,
        public.union_history_candles.data_source,
        public.union_history_candles.guidfromfile
    FROM public.union_history_candles
    group by
        figi
        ,date_trunc('week',public.union_history_candles.candle_start_dt_utc)
        ,public.union_history_candles.guidfromfile
        ,public.union_history_candles.data_source
)
select
    ...
    ,case
        when summary.candle_start_dt_utc
            between date_trunc('week', now()::timestamp)
                and date_trunc('week',now()::timestamp + interval '1 week')
...
drop table if exists public.tmp_week_candle_opcl_prices;
CREATE TABLE public.tmp_week_candle_opcl_prices
(
    figi varchar,
    candle_dt timestamp,
    open_price real,
...
alter table public.tmp_week_candle_opcl_prices
    owner to postgres;

--Заполняем таблицу для хранения цен открытия и закрытия часовых свечей
insert into public.tmp_week_candle_opcl_prices
select
...
select --выбираем минимальный ранг, расчитанный ранее по дневному интервалу
    ...
    ,date_trunc('week', tuhcas.candle_start_dt_utc) as candle_start --вычисляем нужный интервал тут часовой
    ,min(tuhcas.rank_by_week) rank_min -- минимальное значения ранга в интервале - таким образом находим самое первую запись в цепочке
...
on          summary.figi = tuhcas.figi --figi должны соответсвовать
       and  summary.candle_start
                = date_trunc('week',tuhcas.candle_start_dt_utc) --ищем аналогичный интервал в исходных данных
       and  summary.rank_min = tuhcas.rank_by_week --ищем начало интервала по рангу (ранг используем как идентификатор)
) as openPrice_table
...
(
    select
        tuhcas.figi --идентификатор инструмента
        ,date_trunc('week', tuhcas.candle_start_dt_utc) as candle_start --дневной
        ,max(tuhcas.rank_by_week) rank -- максимальное значения ранга в интервале - находим максимальное значение в цепочке
...
on          summary.figi = tuhcas.figi
       and  summary.candle_start = date_trunc('week', tuhcas.candle_start_dt_utc) --ищем аналогичный интервал в исходных данных
       and  summary.rank = tuhcas.rank_by_week ) as closePrice_table) as closePrice_table_sub
...
update tmp_1week_union_history_candles_all_scales t1wuhcas
SET
    open_price = thcop.open_price
    ,close_price = thcop.close_price
from public.tmp_week_candle_opcl_prices thcop
...
from public.tmp_1week_union_history_candles_all_scales t1wuhcas
...
from tmp_1week_union_history_candles_all_scales t1wuhcas
where
...
drop table if exists public.tmp_1week_union_history_candles_all_scales;
