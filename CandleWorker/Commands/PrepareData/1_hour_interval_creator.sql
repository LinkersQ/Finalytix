...
create table if not exists public.tmp_1hour_union_history_candles_all_scales
(
    scale             varchar,
    figi              varchar,
    candle_start_dt_utc   timestamp,
...
comment on column public.tmp_1hour_union_history_candles_all_scales.type_of_candle is 'Свеча восходящая (U) / Свеча нисходящая (D) / Свеча нейтральная (N)';
alter table public.tmp_1hour_union_history_candles_all_scales
    owner to postgres;
...
INSERT INTO public.tmp_1hour_union_history_candles_all_scales
(
    scale,
    figi,
    candle_start_dt_utc,
...
(
    SELECT
        '1_hour_scale' as scale,
        figi,
        date_trunc('hour',public.union_history_candles.candle_start_dt_utc) as candle_start_dt_utc,
        ...
        'script_for_1_hour_intervals_intervals' as source_filename,
    ...
    group by
        figi
        ,date_trunc('hour',public.union_history_candles.candle_start_dt_utc)
        ,public.union_history_candles.guidfromfile
        ,public.union_history_candles.data_source
...
select
    ...
    ,case
        when summary.candle_start_dt_utc
            between date_trunc('hour', now()::timestamp)
                and date_trunc('hour',now()::timestamp + interval '1 hour')
                    then  false --если время свечи попадает в текущий часовой инетрвал считаем что свеча не закрыта
...
drop table if exists public.tmp_hour_candle_opcl_prices;
CREATE TABLE public.tmp_hour_candle_opcl_prices
(
    figi varchar,
    candle_dt timestamp,
    open_price real,
...
alter table public.tmp_hour_candle_opcl_prices
    owner to postgres;

--Заполняем таблицу для хранения цен открытия и закрытия часовых свечей
insert into public.tmp_hour_candle_opcl_prices
select
...
WITH summary AS
(
select --выбираем минимальный ранг, расчитанный ранее по часовому интервалу
    tuhcas.figi --идентификатор инструмента
    ,date_trunc('hour', tuhcas.candle_start_dt_utc) as candle_start --вычисляем нужный интервал тут часовой
    ,min(tuhcas.rank_by_hour) rank_min -- минимальное значения ранга в интервале - таким образом находим самое первую запись в цепочке
...
select --определяем цену открытия свечи для интервала
    tuhcas.figi --идентификатор инструмента
    ,summary.candle_start --15-ти минутный интервал
...
on          summary.figi = tuhcas.figi --figi должны соответсвовать
       and  summary.candle_start
                = date_trunc('hour',tuhcas.candle_start_dt_utc) --ищем аналогичный интервал в исходных данных
       and  summary.rank_min = tuhcas.rank_by_hour --ищем начало интервала по рангу (ранг используем как идентификатор)
) as openPrice_table
...
(
    select
        tuhcas.figi --идентификатор инструмента
        ,date_trunc('hour', tuhcas.candle_start_dt_utc) as candle_start --дневной
        ,max(tuhcas.rank_by_hour) rank -- максимальное значения ранга в интервале - находим максимальное значение в цепочке
...
on          summary.figi = tuhcas.figi
       and  summary.candle_start = date_trunc('hour', tuhcas.candle_start_dt_utc) --ищем аналогичный интервал в исходных данных
       and  summary.rank = tuhcas.rank_by_hour ) as closePrice_table) as closePrice_table_sub
...
update tmp_1hour_union_history_candles_all_scales t1huhcas
SET
    open_price = thcop.open_price
    ,close_price = thcop.close_price
from public.tmp_hour_candle_opcl_prices thcop
where  t1huhcas.figi = thcop.figi
    and t1huhcas.candle_start_dt_utc = thcop.candle_dt;
...
select
    t1huhcas.scale,
    t1huhcas.figi,
    t1huhcas.candle_start_dt_utc,
    t1huhcas.open_price,
    t1huhcas.close_price,
    t1huhcas.max_price,
    t1huhcas.min_price,
    t1huhcas.volume,
    t1huhcas.type_of_candle,
    t1huhcas.is_closed_candle,
    t1huhcas.source_filename,
    t1huhcas.data_source,
    t1huhcas.insertdate_utc,
    t1huhcas.guidfromfile
from public.tmp_1hour_union_history_candles_all_scales t1huhcas
left join public.union_history_candles_all_scales uhcas
    on t1huhcas.figi = uhcas.figi
    and t1huhcas.candle_start_dt_utc = uhcas.candle_start_dt_utc
    and t1huhcas.scale = uhcas.scale
...
set
    max_price = t1huhcas.max_price,
    min_price = t1huhcas.min_price,
    close_price = t1huhcas.close_price,
    is_closed_candle = t1huhcas.is_closed_candle
from tmp_1hour_union_history_candles_all_scales t1huhcas
where
    uhcas.figi = t1huhcas.figi
    and uhcas.candle_start_dt_utc = t1huhcas.candle_start_dt_utc
    and uhcas.is_closed_candle = false --обновляем только незакрытые свечи
    and uhcas.scale = t1huhcas.scale;

--удаляем использованную таблицу с данными по часовым интервалам
drop table if exists public.tmp_1hour_union_history_candles_all_scales;
